...
func TestToManifest(t *testing.T) {
	...
	if len(repo.Spec.BranchProtection.Get()) != 1 {
		t.Fatalf("BranchProtection count = %d, want 1", len(repo.Spec.BranchProtection.Get()))
	}
	bp := repo.Spec.BranchProtection.Get()[0]
	if bp.Pattern != "main" {
...
func TestToManifest_NoBranchProtection(t *testing.T) {
	...
	if len(repo.Spec.BranchProtection.Get()) != 0 {
		t.Errorf("expected 0 branch protections, got %d", len(repo.Spec.BranchProtection.Get()))
	}
...
func TestToManifest_NilStatusChecks(t *testing.T) {
	...
	if len(repo.Spec.BranchProtection.Get()) != 1 {
		t.Fatalf("expected 1 branch protection, got %d", len(repo.Spec.BranchProtection.Get()))
	}
	if repo.Spec.BranchProtection.Get()[0].RequireStatusChecks != nil {
...
func TestToManifest_Rulesets(t *testing.T) {
	...
	if len(repo.Spec.Rulesets.Get()) != 1 {
		t.Fatalf("expected 1 ruleset, got %d", len(repo.Spec.Rulesets.Get()))
	}

	rs := repo.Spec.Rulesets.Get()[0]
