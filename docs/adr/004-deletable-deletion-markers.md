...
The product-level requirement from ADR-003 remains valid:
...
The implementation should preserve gh-infra's stateless design, avoid a YAML library fork, and keep parser knowledge of individual repository fields low.
...
Adopt `Deletable[T]` and parser-side marker recovery. Keep the user-facing YAML semantics from ADR-003, but reject the `Nullable[T]` plus YAML fork implementation.
